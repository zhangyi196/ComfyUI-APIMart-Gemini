...
class CPAGPTImage2GenerationNode:
    ...
    def upload_reference_images(
        ...
        for index, image_tensor in enumerate(reference_images, start=1):
            image_urls.append(self.upload_image(image_tensor, api_key, upload_url, index))
        return image_urls
    ...
    def poll_task_status(self, task_id: str, api_key: str, query_url: str) -> Tuple[str, Dict[str, Any]]:
        ...
        for poll_count in range(1, self.max_polls + 1):
            try:
                response = requests.get(f"{query_url}/{task_id}", headers=headers, timeout=15)
                response.raise_for_status()
                response_data = response.json()
                print(
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            image_urls = (
                self.upload_reference_images(reference_images, upload_api_key, upload_url)
                if reference_images
                else []
            )
